...
votings_create_table <- function(dbname,user,password,host,home_page,page){
  ...
  meetings_table <- votings_get_meetings_table(home_page,page)
  ...
  for(i in seq_len(length(meetings_links))){
    ...
    votings_links <- votings_get_votings_links(meetings_links[i])
    
    #putting this data frame to database
    drv <- dbDriver("PostgreSQL")
    database_diet <- dbConnect(drv,dbname=dbname,user=user,password=password,host=host)
    for(j in seq_len(length(votings_links))){
